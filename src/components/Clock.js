...
class Clock extends React.Component {
    constructor(props) {
       super(props);
       this.time = props.timeZone;
       this.ref = React.createRef();
       this.state = { time: new Date(new Date().toLocaleString("en-US", {timeZone: this.time}).toString()) };
    ...
    getDrawingContext() {
       this.drawingContext = this.ref.current.getContext('2d');
       this.drawingContext.translate(this.radius, this.radius);
       this.radius *= 0.9;
    }
 
    tick() {
       this.setState({ time: new Date(new Date().toLocaleString("en-US", {timeZone: this.time}).toString()) });
    ...
    render() {
       ...
       return (
          <div className="Clock" style={{ width: String(this.props.size) + 'px' }}>
             ...
             <canvas width={this.props.size} height={this.props.size} ref={this.ref}/>
             <div className={ClassNamed({
                     darkModeText : darkMode === 'on'
                  })}>
               <h1>{this.time}</h1>
             </div>
